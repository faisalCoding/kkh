 ...
 {{--  --}}
        ...
        <div class="w-5/12 flex flex-col mx-auto items-center bg-white rounded-xl z-10 goup overflow-hidden">
            <div class="flex justify-center w-11/12">
                <div class="flex flex-col w-6/12">
                    <h1 class="py-3 text-lg text-blue-600 p-5 bg-blue-100 bg-opacity-30 rounded-lg m-1">انت فعلا مسجل لديا باسم <span class="bg-blue-200 rounded-lg m-1"> {{ $nameOfUser }}</span></h1>
                    <h1 class="py-3 text-md text-yellow-600 p-5 bg-yellow-100 bg-opacity-30 rounded-lg m-1" bg-yellow-100 >قم بادخال كلمة المرور المسجلة بهذا الرقم <span class="bg-white text-gray-400 rounded-lg m-1">{{ $new_user_phone }}</span></h1>

                    
                    <form wire:submit.prevent="submit" method="POST" method="POST" id="form" class="m-2" action="{{ route('login', config('app.locale')) }} " dir="{{ config('app.locale') == 'ar' ? 'rtl' : 'ltr';}}">
                        @csrf
                        
                        <div>
                            <input id="phone" hidden value="{{ $new_user_phone }}" type="text" name="phone" />
                            
                        </div>
                        <div class="mt-4">
                            <x-jet-label for="password" class="py-3 text-lg text-gray-700 " value="{{ __('Password') }}" />
                            <x-jet-input id="password" class="block mt-1 w-full" type="password" wire:model="password" required autocomplete="current-password" />
                        </div>
